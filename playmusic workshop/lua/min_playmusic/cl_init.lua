...
.preloader4 {
   ...
   border-right-color:rgba(100,100,100, 255);
...
<div class="preloader4"></div>
	...
	html:SetHTML( dddddddd )

	
	timer.Simple( 0.5, function()
		html:OpenURL("https://cdn.rawgit.com/Minbird/minbird.github.io/b6076144/html/app/youtube.html?uri=" .. MPlayM.URI) -- https://cdn.rawgit.com/Minbird/minbird.github.io/b6076144/html/app/youtube.html
	end)
	
	html:AddFunction("MinPlaymusic", "PlayerState", function(state)
	local PlayerState = tostring(state)
	
		MPlayM.NoConnection = true
		
		if PlayerState == "5" then -- solved?
		
			MPlayM.NoConnection = false
	...
	timer.Simple( 10, function()
		if MPlayM.NoConnection then
			MPlayM.err("[Host Connection failed: Time out.] Try to connect to the local player.")
			html:OpenURL("asset://garrysmod/lua/min_playmusic/host/host.html?uri=" .. MPlayM.URI)
		end
	end)
...
MsModules.Playmusic = function(ply, text, teamchat, isdead, station, c, args)
	...
	if MsModules.CallPlaymusic(text) and ply == LocalPlayer() then
		...
		if string.len(command) == 0 then
			chat.AddText(Color(255, 255, 255), PlayMLanguage.howToUse)
		...
		elseif command == "vol" then
			if string.len(args) == 0 then
				...
				if not tonumber(args) then
					MPlayM.n(tostring(args) .. PlayMLanguage.PleaseEnterAnInteger)
					return
				end
				MPlayM.vol = math.Clamp( tonumber(args), 0, 100 )
